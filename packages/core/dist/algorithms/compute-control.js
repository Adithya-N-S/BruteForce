 ...
 */
export function computeControl(paths, options) {
    ...
    for (const pathResult of paths) {
        ...
        for (const edge of path) {
            ...
            if (isNaN(val) || val < 0 || val > 1) {
                throw new RangeError(`Invalid ownership percentage value: ${val} on edge ${edge.id}. Value must be in range [0, 1].`);
            }
            pathControl *= val;
        }
        // Summing control
    ...
    if (contributingPaths.length === 0) {
        ...
        explanation = `Total effective control is ${controlStr}% (${thresholdReached ? "meets" : "does not meet"} ${thresholdStr}% threshold). `;
...
//# sourceMappingURL=compute-control.js.map