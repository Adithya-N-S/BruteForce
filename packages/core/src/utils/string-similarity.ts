 ...
 */
const DIACRITICS_MAP: Readonly<Record<string, string>> = {
 ...
 */
const LEGAL_SUFFIXES: readonly RegExp[] = [
...
export function jaroWinklerSimilarity(
  s1: string,
  s2: string,
  prefixScale: number = WINKLER_PREFIX_SCALE,
): number {
 ...
 */
export function isNormalizedExactMatch(a: string, b: string): boolean {
 ...
 */
export function hasIdentifierOverlap(
  queryIds: readonly string[],
  candidateIds: readonly string[],
): boolean {
  if (queryIds.length === 0 || candidateIds.length === 0) return false;

  const normalizedCandidates = new Set(
    candidateIds.map((id) => normalizeString(id)),
  );
