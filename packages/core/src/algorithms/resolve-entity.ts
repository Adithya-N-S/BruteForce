...
/**
 * Per-field scoring result. Tracks the similarity value and the
 * human-readable feature description for the explanation.
 */
interface FieldScore {
  /** Field name (matches FIELD_WEIGHTS key). */
  readonly field: keyof typeof FIELD_WEIGHTS;

  /** Similarity score for this field in [0, 1]. */
  readonly score: number;

  /**
   * Human-readable feature tag (e.g., `'name:jaro=0.94'`).
   * Only present when the score is > 0.
   */
 ...
 */
function scoreNameField(queryName: string, candidateName: string): FieldScore {
  // 1. Exact raw match
 ...
 */
function extractIdentifiers(entity: EntityNode): string[] {
  const ids: string[] = [];
 ...
 * @returns The field score.
 */
function scoreAddressField(queryAddress: string, entity: EntityNode): FieldScore {
 ...
 */
function extractAddress(entity: EntityNode): string | null {
 ...
 * @returns The field score.
 */
function scoreJurisdictionField(
  queryJurisdiction: string,
  candidateJurisdiction: string,
): FieldScore {
  if (
    normalizeString(queryJurisdiction).length > 0 &&
 ...
 */
function isAmbiguous(score: number): boolean {
    ...
    switch (fs.field) {
      case 'name':
        if (fs.feature === 'name:exact') {
          ...
          parts.push(
            `Jaro-Winkler name similarity ${fs.score.toFixed(2)} on '${entityName}'`,
          );
        }
        break;
      case 'identifiers':
...
export function resolveEntity(
  ...
  const hasIdentifiers =
    query.identifiers !== undefined && query.identifiers.length > 0;
  const hasJurisdiction =
    query.jurisdiction !== undefined && query.jurisdiction.trim().length > 0;
  ...
  const activeWeights: Partial<Record<keyof typeof FIELD_WEIGHTS, number>> = {};
  if (hasName) activeWeights.name = FIELD_WEIGHTS.name;
  if (hasIdentifiers) activeWeights.identifiers = FIELD_WEIGHTS.identifiers;
  if (hasAddress) activeWeights.address = FIELD_WEIGHTS.address;
  if (hasJurisdiction) activeWeights.jurisdiction = FIELD_WEIGHTS.jurisdiction;
  ...
  const matches: ResolveEntityMatch[] = [];
  ...
  for (const entity of entities) {
    ...
    if (hasJurisdiction) {
      fieldScores.push(
        scoreJurisdictionField(query.jurisdiction!, entity.jurisdiction),
      );
    }
    ...
    // Apply threshold
    if (compositeScore < MIN_SCORE_THRESHOLD) continue;

    // Collect matched features (non-null features only)
    const matched_features = fieldScores
      .filter((fs): fs is FieldScore & { feature: string } => fs.feature !== null)
      .map((fs) => fs.feature);
